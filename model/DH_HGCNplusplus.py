...
from base.multipleRelation import multipleRelation, buildItemRelation
import os
...
class DH_HGCNplusplus(SocialRecommender, DeepRecommender):
    ...
    def readConfiguration(self):
        super(DH_HGCNplusplus, self).readConfiguration()
        args = config.LineConfig(self.config['DH_HGCNplusplus'])
    ...
    def buildSparseGRelationMatrix(self):
        row, col, entries = [], [], []
        a = multipleRelation()
        grouprelation = a.buildGroupRelation(abspath(self.config['group']))
        for pair in grouprelation:
            try:
                if pair[0] in self.data.user.keys() and pair[1] in self.data.user.keys():
                    row += [self.data.user[pair[0]]]
                    col += [self.data.user[pair[1]]]
                ...
                pass
        GAdjacencyMatrix = coo_matrix((entries, (row, col)), shape=(self.num_users, self.num_users), dtype=np.float32)
        return GAdjacencyMatrix
    ...
    def buildMotifInduceAdjacencyMatrix(self):
        ...
        G = self.buildSparseGRelationMatrix()
        ...
        #GROUP
        B2=G.multiply(G.T)
        A8=(B2.dot(B2)).multiply(B2)
        #COMMENT
        B3 = C.multiply(C.T)
        A9 = (B3.dot(B3)).multiply(B3)
        #F
        A10 = (R.dot(R.T)).multiply(B)
        A11 = (R.dot(R.T)).multiply(U)
        A11 = A11 + A11.T
        #G
        A12 = (R.dot(R.T)).multiply(B2)
        #C
        A13 = (R.dot(R.T)).multiply(B3)
        A14 = R.dot(R.T) - A10 - A11-A12-A13
        #ITEM
        A15 = (R.T).dot(R)
        ...
        H_g = H_g.multiply(1.0 / H_g.sum(axis=1).reshape(-1, 1))
        H_c = A9

        H_c = H_c.multiply(1.0 / H_c.sum(axis=1).reshape(-1, 1))
        H_p = sum([A10, A11, A12, A13, A14])
    ...
    def initModel(self):
        super(DH_HGCNplusplus, self).initModel()
        ...
        H_g = total[2]
        H_g = self.adj_to_sparse_tensor(H_g)
        H_p = total[3]
        ...
        interaction_user_embeddings = self_gating(self.user_embeddings, 4)

        all_embeddings_c1 = [user_embeddings_c1]
        all_embeddings_c2 = [user_embeddings_c2]
        all_embeddings_c3 = [user_embeddings_c3]
        all_embeddings_interaction = [interaction_user_embeddings]
        ...
        for k in range(self.n_layers):
            ...
            user_embeddings_c3 = tf.sparse_tensor_dense_matmul(H_g, user_embeddings_c3)
            ...
            all_embeddings_interaction += [tf.math.l2_normalize(interaction_user_embeddings, axis=1)]
        ...
        interaction_user_embeddings = tf.reduce_sum(all_embeddings_interaction, axis=0)
        ...
        new_item_embeddings = item_embeddings_rating + item_embeddings_fea
        # aggregating
        ...
        self.cnt_loss += self.cal_contrastive_loss(user_embeddings_c3,interaction_user_embeddings)

    def cal_contrastive_loss(self, pos, emb):
        def row_shuffle(embedding):
            return tf.gather(embedding, tf.random.shuffle(tf.range(tf.shape(embedding)[0])))
        pos_score_pos = tf.reduce_sum(tf.multiply(pos, emb),axis=1)
        ttl_score_neg = tf.matmul(pos, row_shuffle(emb), transpose_a=False,transpose_b=True)
        pos_score = tf.exp(pos_score_pos / 0.2)
        ttl_score = tf.reduce_sum(tf.exp(ttl_score_neg / 0.2), axis=1)
        ctl_loss = -tf.reduce_sum(tf.log(pos_score/ttl_score))
        return ctl_loss
