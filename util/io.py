...
class FileIO(object):
    ...
    @staticmethod
    def loadCommentRelationship(conf, filePath):
        commentConfig = LineConfig(conf['comment.setup'])
        commentrelation = []
        print('loading comment data...')
        with open(filePath) as f:
            relations = f.readlines()
            # ignore the headline
        if commentConfig.contains('-header'):
            relations = relations[1:]
        # order of the columns
        order = commentConfig['-columns'].strip().split()
        if len(order) <= 2:
            print('The commentrelation file is not in a correct format.')
        for lineNo, line in enumerate(relations):
            items = split(' |,|\t', line.strip())
            if len(order) < 2:
                print('The social file is not in a correct format. Error: Line num %d' % lineNo)
