...
class Measure(object):
    ...
    def rankingMeasure(origin, res, rawRes, N):
        ...
        for n in N:
            ...
            # indicators.append('AUC:' + str(AUC) + '\n')
            # -----------
            HR = Measure.HR(hits, origin)
            indicators.append('HR:' + str(HR) + '\n')
    ...
    def AUC(origin, res, rawRes):
        ...
        for user in origin:
            count = 0
            larger = 0
            itemList = list(rawRes[user].keys())
    ...
    def HR(hits, origin):
        ...
        originsum = 0
        for user in hits:
            hitsum = hits[user] + hitsum
            originsum = len(origin[user]) + originsum
